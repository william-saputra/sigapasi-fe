...
export const useTimeSlotStore = defineStore('timeSlot', () => {
  ...
  async function fetchSchoolLevels(): Promise<void> {
    ...
    try {
      ...
      error.value =
        err.response?.data?.message || err.message || 'Gagal mengambil data jenjang pendidikan.'
    }
  }
  ...
  async function createAcademicYear(
    payload: CreateAcademicYearRequestDTO,
    silentError = false,
    ...
    try {
      ...
      const apiMessage = err.response?.data?.message || 'Gagal membuat tahun ajaran baru.'
      if (!silentError && toast) toast.error(apiMessage)
      return { success: false, error: apiMessage }
    }
  }

  async function createSemester(
    payload: CreateSemesterRequestDTO,
    silentError = false,
  ): Promise<{ success: boolean; error?: string }> {
    error.value = null
    try {
      ...
      const apiMessage = err.response?.data?.message || 'Gagal membuat semester baru.'
      if (!silentError && toast) toast.error(apiMessage)
      return { success: false, error: apiMessage }
    }
  }
  ...
  async function fetchSlotStructureForAll(): Promise<void> {
    ...
    try {
      ...
      data.forEach((dto: any) => {
        ...
        const schoolLevelId =
          dto.schoolLevelId !== undefined ? dto.schoolLevelId : dto.school_level_id
        const schoolLevelName =
          dto.schoolLevelName !== undefined ? dto.schoolLevelName : dto.school_level_name
      ...
      error.value =
        err.response?.data?.message || err.message || 'Gagal mengambil data semua tingkatan.'
    } finally {
  ...
  async function saveSlotStructure(
    ...
    try {
      ...
      if (hasInvalidTime) {
        throw new Error(
          'Akumulasi waktu berakhir melebihi jam 23:59. Silakan sesuaikan kembali jadwal.',
        )
      }
      ...
      const applyLocks = async (gradeData: any[]) => {
        ...
        for (const lock of pendingLocks) {
          const matchingDto = gradeData.find((dto: any) => {
            const startStr = dto.startTime || dto.start_time
            return (
              parseIsoTime(startStr) === lock.start_time &&
      ...
      if (applyToAllLevels) {
        for (const level of schoolLevels.value) {
          ...
          try {
            ...
            if (err.response?.status === 409) {
              console.warn(
                `Pengaturan slot waktu pada semester dan tingkat ${level.id} ini sudah ada (409 Conflict).`,
              )
            }
            throw err
          }
        ...
        try {
          ...
          if (err.response?.status === 409) {
            console.warn(
              'Pengaturan slot waktu pada semester dan tingkat ini sudah ada (409 Conflict).',
            )
          }
          throw err
        }
      ...
      const apiMessage =
        err.response?.data?.message || err.message || 'Terjadi konflik jadwal. Gagal menyimpan.'
