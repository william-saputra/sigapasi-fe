...
function onStartEdgePull(entry: ScheduleEntryDTO, slotId: string) {
  const slot = workspaceStore.filteredTimeSlots[
    ALL_DAYS.find((d) => workspaceStore.filteredTimeSlots[d]?.some((s) => s.id === slotId)) as
      | DayOfWeek
      | undefined
  ]?.find((s) => s.id === slotId)

  const day =
    ALL_DAYS.find((d) => workspaceStore.filteredTimeSlots[d]?.some((s) => s.id === slotId)) ?? ''
