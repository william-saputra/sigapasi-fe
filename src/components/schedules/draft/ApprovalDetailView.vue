...
import type { ScheduleApprovalListDTO, ScheduleDraftDTO } from '@/interfaces/schedules/schedule.types'
...
<template>
  <div>
    <div class="mb-6 flex flex-col gap-4 rounded-xl bg-white p-5 shadow-sm border border-gray-200">
      ...
      <div class="flex flex-wrap items-center justify-between gap-4">
        <div class="flex items-center gap-4">
          ...
          <div>
            <div class="flex items-center gap-3 mb-1">
              ...
              <span :class="['rounded-full px-3 py-0.5 text-xs font-semibold border', statusBadge(approvalData.status)]">
                {{ statusLabel(approvalData.status) }}
              </span>
            </div>
            <span class="text-xs text-gray-500 font-medium">
              Pengaju: {{ approvalData.createdBy }} • Diajukan: {{ new Date(approvalData.createdAt).toLocaleDateString('id-ID') }}
            </span>
          </div>
        </div>
        ...
        <div class="flex gap-3">
          ...
          >
            {{ approvalData.status === 'PUBLISHED' ? '❌ Batal Publikasi & Revisi' : '❌ Minta Revisi' }}
          </button>
      ...
      <div v-if="approvalData.status === 'REVISION_REQUIRED' && approvalData.revisionNote" class="w-full rounded-lg bg-red-50 p-3 border border-red-100 mt-2">
    ...
    >
      <div class="inline-block h-10 w-10 animate-spin rounded-full border-4 border-emerald-200 border-t-emerald-700"></div>
      <p class="mt-4 text-sm font-medium text-gray-500">Memuat detail jadwal...</p>
    </div>

    <div v-else class="flex gap-4 items-start">

      <div class="w-64 shrink-0 rounded-xl bg-white p-4 shadow-sm border border-gray-200 sticky top-4">
        ...
        <div class="flex flex-col gap-2 max-h-[calc(100vh-240px)] overflow-y-auto pr-1">
          <button
            ...
            :class="workspaceStore.activeClassId === cls.classId
      ...
      <div class="flex-1 min-w-0">
        ...
        >
          <div class="absolute inset-0 z-10 cursor-not-allowed" title="Mode Pratinjau (Read-Only)"></div>
        ...
        <div v-else class="rounded-xl border border-gray-200 bg-white py-20 text-center shadow-sm">
          ...
          <p class="text-sm text-gray-500 max-w-md mx-auto">
            Silakan pilih kelas pada panel di kiri untuk melihat susunan jadwal sebelum Anda memberikan persetujuan atau meminta revisi.
          </p>
        </div>
    ...
    >
      <div class="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl">
        <h3 class="mb-2 text-xl font-bold text-gray-800">
          {{ approvalData.status === 'PUBLISHED' ? 'Tarik Publikasi & Minta Revisi' : 'Minta Revisi Jadwal' }}
        </h3>
        <p class="mb-5 text-sm text-gray-500">
          <span v-if="approvalData.status === 'PUBLISHED'">
            Jadwal ini akan <strong>ditarik dari publikasi</strong> dan dikembalikan ke Staff dengan status <span class="font-bold text-red-600">Butuh Revisi</span>.
          </span>
          <span v-else>
            Jadwal ini akan dikembalikan ke Staff dengan status <span class="font-bold text-red-600">Butuh Revisi</span>. 
          </span>
        ...
        <div class="mt-6 flex justify-end gap-3">
          <button
            ...
            @click="showRevisionModal = false; revisionNote = ''"
          ...
          >
            {{ isSubmitting ? 'Mengirim...' : (approvalData.status === 'PUBLISHED' ? 'Ya, Unpublish & Revisi' : 'Kirim Catatan Revisi') }}
          </button>
        </div>
      </div>
    </div>
    ...
    >
      <div class="w-full max-w-md rounded-2xl bg-white p-7 shadow-2xl text-center transform transition-all">
        ...
        <p class="mb-8 text-sm text-gray-500 leading-relaxed">
          Jadwal <span class="font-bold text-gray-700">"{{ approvalData.scheduleName }}"</span> akan diterbitkan dan menjadi jadwal resmi. Tindakan ini tidak dapat dibatalkan.
        </p>
        ...
        <div class="flex justify-center gap-3">
          ...
          <button
            ...
            @click="submitApprove"
          >
            <span v-if="isApproving" class="mr-2 inline-block h-4 w-4 animate-spin rounded-full border-2 border-white border-t-transparent"></span>
