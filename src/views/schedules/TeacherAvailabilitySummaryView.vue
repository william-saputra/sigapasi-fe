...
import { Calendar, CheckCircle2, XCircle, Clock, ListChecks, Activity, Pencil, Lock, Coffee, HelpCircle } from 'lucide-vue-next'
...
<template>
    <div class="p-3 md:p-5 max-w-4xl mx-auto flex flex-col gap-4">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <div class="bg-white p-3 rounded-lg border border-gray-200 flex flex-col justify-between shadow-sm">
                ...
                <div class="text-xs font-bold text-gray-700 truncate mt-1">
                    {{ timeSlotStore.academicYears.find(ay => ay.listSemesters.some(s => s.id === timeSlotStore.activeSemesterId))?.yearStart }}/{{ timeSlotStore.academicYears.find(ay => ay.listSemesters.some(s => s.id === timeSlotStore.activeSemesterId))?.yearEnd }}
                </div>
            </div>
        ...
        <div v-else class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div class="flex border-b border-gray-100 bg-gray-50">
                <button 
                    v-for="day in ALL_DAYS" :key="day"
                    @click="selectedDay = day"
                    :class="[
                        'flex-1 py-2 text-[11px] font-bold transition-all outline-none border-b-2',
                        selectedDay === day ? 'bg-white border-blue-600 text-blue-700' : 'border-transparent text-gray-500 hover:bg-gray-100'
                    ]"
                >
            ...
            <div class="flex flex-col">
                <div v-if="slotsForSelectedDay.length === 0" class="py-12 text-center text-gray-400 text-xs font-medium">
                    Tidak ada jadwal tersedia.
                </div>
                
                <div v-else class="flex flex-col divide-y divide-gray-50">
                    <div 
                        v-for="slot in slotsForSelectedDay" :key="slot.id" 
                        class="flex items-center justify-between p-3 px-4 hover:bg-gray-50/50 transition-colors"
                        :class="{'opacity-50 grayscale bg-gray-50': slot.slot_type === 'BREAK' || slot.is_locked}"
                    >
                        <div class="flex items-center gap-4">
                            ...
                            <span v-if="slot.slot_type === 'BREAK'" class="text-[10px] font-bold text-gray-400 flex items-center gap-1">
                                <Coffee :size="10" /> ISTIRAHAT
                            </span>
                            <span v-else class="text-[10px] font-bold text-blue-600 bg-blue-50 px-1.5 py-0.5 rounded border border-blue-100">
                        ...
                        <div class="flex items-center">
                            ...
                            <div v-else-if="slot.is_locked" class="text-[10px] font-bold text-slate-400 bg-slate-100 px-2 py-0.5 rounded-full flex items-center gap-1">
                                <Lock :size="10" /> LOCKED
                            </div>

                                <div v-else-if="slot.id && availabilityMap[slot.id] === true" class="text-[10px] font-bold text-green-700 bg-green-50 px-3 py-1 rounded-full border border-green-200 flex items-center gap-1">
                                    <CheckCircle2 :size="12" /> BISA
                                </div>

                                <div v-else-if="slot.id && availabilityMap[slot.id] === false" class="text-[10px] font-bold text-red-700 bg-red-50 px-3 py-1 rounded-full border border-red-200 flex items-center gap-1">
                                    <XCircle :size="12" /> TIDAK
                                </div>
                            <div v-else class="text-[10px] font-bold text-gray-400 bg-gray-50 px-3 py-1 rounded-full border border-gray-200 border-dashed flex items-center gap-1">
