...
import { Calendar, Save, AlertCircle, CheckCircle2, XCircle, Clock, ChevronDown, CheckSquare, ListChecks, Activity } from 'lucide-vue-next'
...
const totalAvailableSlots = computed(() => Object.values(availabilityMap.value).filter(s => s === true).length)

const fillPercentage = computed(() => totalValidSlots.value === 0 ? 0 : Math.round((totalFilledSlots.value / totalValidSlots.value) * 100))
const availablePercentage = computed(() => totalFilledSlots.value === 0 ? 0 : Math.round((totalAvailableSlots.value / totalValidSlots.value) * 100))
...
<template>
    <div class="p-3 md:p-5 max-w-5xl mx-auto flex flex-col gap-4">
        ...
        <Transition name="fade">
            <div v-if="message.text" :class="`p-2.5 rounded-md flex items-center gap-2 font-medium text-xs border ${message.type === 'success' ? 'bg-green-50 text-green-700 border-green-200' : 'bg-red-50 text-red-700 border-red-200'}`">
        ...
        <div class="grid grid-cols-1 md:grid-cols-12 gap-3">
            ...
            <div class="md:col-span-5 bg-white p-3 rounded-lg border border-gray-200 flex flex-col justify-center gap-2">
                <div class="flex flex-col sm:flex-row items-start sm:items-center gap-2">
                    ...
                    >
                        <optgroup v-for="ay in timeSlotStore.academicYears" :key="ay.id" :label="`${ay.yearStart}/${ay.yearEnd}`">
                ...
                <div v-if="deadlineDate" :class="`inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded text-[11px] font-semibold border ${isEditable ? 'bg-amber-50 border-amber-200 text-amber-800' : 'bg-red-50 border-red-200 text-red-800'}`">
                    <Calendar :size="14" />
                    <span>{{ isEditable ? 'Batas Isi:' : 'Tenggat Habis:' }} {{ new Date(deadlineDate).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' }) }}</span>
                </div>
            </div>

            <div class="md:col-span-4 bg-slate-50 p-3 rounded-lg border border-slate-200 flex flex-col justify-between">
                <div class="flex justify-between items-center mb-1">
                    ...
                    <span class="text-[10px] font-bold text-slate-600">{{ totalFilledSlots }} / {{ totalValidSlots }} Slot</span>
                </div>
                <div class="w-full bg-slate-200 rounded-full h-1.5 mb-1.5">
                    <div class="bg-slate-600 h-1.5 rounded-full transition-all duration-500" :style="`width: ${fillPercentage}%`"></div>
                </div>
                <div class="text-[10px] text-slate-500 font-medium text-right">{{ fillPercentage }}% Selesai</div>
            </div>

            <div class="md:col-span-3 bg-blue-50 p-3 rounded-lg border border-blue-100 flex flex-col justify-between">
                <div class="flex justify-between items-center mb-1">
                ...
                <div class="w-full bg-blue-200 rounded-full h-1.5 mb-1.5">
                    <div class="bg-blue-500 h-1.5 rounded-full transition-all duration-500" :style="`width: ${availablePercentage}%`"></div>
                </div>
                <div class="text-[10px] text-blue-600 font-medium text-right">{{ availablePercentage }}% Ketersediaan</div>
            </div>
        ...
        <div v-else class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div class="flex overflow-x-auto border-b border-gray-200 bg-gray-50 scrollbar-hide">
                <button 
                    ...
                    :key="day"
                    @click="selectedDay = day; openSlotId = null"
                    :class="[
            ...
            <div class="p-4 bg-gray-50/50 min-h-[300px]">
                ...
                <div v-if="slotsForSelectedDay.length === 0" class="text-center py-10 text-gray-400 text-xs">
                ...
                <div v-else class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2.5">
                    ...
                    <div 
                        ...
                        :class="slot.slot_type === 'BREAK' || slot.is_locked ? 'border-gray-200 opacity-60 bg-gray-50' : 'border-gray-200 hover:border-blue-300'"
                    >
                        ...
                        <div v-if="!slot.id" class="h-7 flex items-center justify-center text-[9px] text-amber-600 bg-amber-50 rounded mt-auto">
                            Menunggu
                        </div>
                        
                        <div v-else-if="slot.slot_type === 'BREAK' || slot.is_locked" 
