...
async function onMouseUp() {
  ...
  if (result.warnings && result.warnings.length > 0) {
    const warningMsg = result.warnings
      .map((w: any) => {
        const day = ALL_DAYS.find((d) => workspaceStore.filteredTimeSlots[d as DayOfWeek]?.some((s: any) => s.id === w.slotId)) as DayOfWeek | undefined
      ...
      .join(', ')
    emit('show-toast', `Bentrok di slot ${warningMsg}: ${result.warnings[0]?.message || 'Terjadi bentrok'}`, 'warning')
  }
