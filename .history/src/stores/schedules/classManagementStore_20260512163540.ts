...
export const useClassManagementStore = defineStore('classManagement', () => {
    ...
    async function fetchTargetsForSelectedClass() {
        ...
        try {
            ...
            if (allSubjectsByLevel.value.length === 0 || allSubjectsByLevel.value[0]?.schoolLevelId !== selectedClass.value.schoolLevelId) {
    ...
    async function saveAllConfig(editedTargets: { id?: string; subjectId: string; targetHours: number }[]): Promise<boolean> {
