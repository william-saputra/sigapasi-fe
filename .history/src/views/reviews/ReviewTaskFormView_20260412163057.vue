...
const aspectFields: Array<{ key: keyof Omit<ReviewSubmitRequest, 'taskId' | 'comment'>; label: string }> = [
...
async function submitReview() {
  ...
  try {
    ...
    if (status === 400) {
      submitError.value = 'Data penilaian tidak valid. Pastikan semua nilai sesuai rentang 1 sampai 5.'
    } else if (status === 403) {
      submitError.value = 'Anda tidak memiliki akses untuk mengirim tugas ini atau tugas sudah completed.'
    } else if (status === 404) {
