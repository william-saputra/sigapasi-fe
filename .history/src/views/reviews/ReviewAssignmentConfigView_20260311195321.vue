...
const superiors = ref<ReviewerCandidate[]>([])
const peers = ref<ReviewerCandidate[]>([])
const isLoadingReviewers = ref(false)

async function fetchReviewers() {
  isLoadingReviewers.value = true
  try {
    const [sup, peerList] = await Promise.all([
      apiService.get<ReviewerCandidate[]>('/teachers/reviewers/superiors'),
      apiService.get<ReviewerCandidate[]>(`/teachers/reviewers/peers?exclude_id=${teacherId}`),
    ])
    superiors.value = sup.filter((r) => r.role === 'HEAD')
...
const filteredPeers = computed(() => {
  if (!peerSearch.value.trim()) return peers.value
  return peers.value.filter((r) =>
    r.fullName.toLowerCase().includes(peerSearch.value.toLowerCase()),
  )
})
    ...
    <div class="config-layout">
          ...
          <template v-else>
            ...
            <div class="form-group">
              ...
              >
                ...
                <option v-for="r in superiors" :key="r.id" :value="r.id">
                  {{ r.fullName }} ({{ r.role }})
                </option>
              </select>
              <div v-if="errors.superior" class="error-message">Superior wajib dipilih.</div>
            </div>
            ...
            <div class="form-group">
              ...
              <div v-if="errors.peers" class="error-message">Pilih minimal 1 guru sebaya.</div>
            </div>

            <!-- Submit -->
            <button type="button" class="btn-publish" :disabled="isSubmitting" @click="submit">
      ...
      <div class="note-card">
        ...
        <p class="note-card__body">
          Setelah tombol <strong>Publish</strong> ditekan, akses form evaluasi akan terbuka untuk
        ...
        <ul class="note-card__list">
          ...
          <li>
            Guru yang bersangkutan akan melakukan <strong>penilaian mandiri</strong> (self-review).
          </li>
