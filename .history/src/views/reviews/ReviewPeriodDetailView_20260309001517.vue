...
async function fetchMapping(teacherId: string) {
  ...
  try {
    ...
    mappingCache.value[teacherId] = { status: 'loaded', superiorName: data.superiorName, peers: data.peers }
  } catch {
...
<template>
  <AppLayout>
    ...
    <AppCard>
      ...
      <DataTable v-else :columns="columns" :rows="filteredTeachers" expandable @expand="fetchMapping">
        <!-- Nama Guru -->
            ...
            <template v-else-if="mappingCache[row.teacherId]?.status === 'loaded'">
              <div class="mapping-row">
                ...
                <span class="mapping-value">
                  {{
                    (mappingCache[row.teacherId] as { status: 'loaded'; superiorName: string | null; peers: any[] }).superiorName ??
              ...
              <div class="mapping-row">
                ...
                <span
                  v-if="(mappingCache[row.teacherId] as { status: 'loaded'; superiorName: string | null; peers: { id: string; fullName: string }[] }).peers.length === 0"
                ...
                <div v-else class="mapping-peers">
                  <span
                    v-for="peer in (mappingCache[row.teacherId] as { status: 'loaded'; superiorName: string | null; peers: { id: string; fullName: string }[] }).peers"
                    :key="peer.id"
                    class="peer-chip"
                  >
                    {{ peer.fullName }}
