...
const columns = [
  { key: 'namaGuru', label: 'Nama Guru', thStyle: 'width: 35%;' },
  { key: 'jabatan', label: 'Jabatan', thStyle: 'width: 20%;' },
  { key: 'status', label: 'Status', thStyle: 'width: 20%;' },
  { key: 'aksi', label: 'Aksi', thStyle: 'width: 25%;' },
]
...
async function submitAddTeachers() {
  ...
  if (successCount > 0) {
    toast.success(`${successCount} guru berhasil ditambahkan ke periode.`)
    await fetchTeachers()
  }
  if (failCount > 0) {
...
<template>
  <AppLayout>
    ...
    <AppCard>
      ...
      <DataTable v-else :columns="columns" :rows="filteredTeachers">
        <!-- Nama Guru -->
        ...
        <template #cell:aksi="{ row }">
          <button v-if="row.status === 'DRAFT'" class="btn-outline">
            ⚙️ Atur Penilai
          </button>
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            ...
            <div v-else-if="filteredCandidates.length === 0" class="candidate-state">
              <span>{{ candidateSearch ? 'Tidak ada hasil.' : 'Semua guru sudah ditambahkan.' }}</span>
            </div>
