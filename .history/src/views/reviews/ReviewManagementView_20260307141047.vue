...
// ─── Semester Options (from GET /api/semesters) ───────────────────────────────
...
async function fetchSemesters() {
  ...
  try {
    const data = await apiService.get<SemesterOption[]>('/semesters')
    semesterOptions.value = data
  } catch {
    // non-critical — form will show empty dropdown with error hint
  } finally {
      ...
      <div class="modal">
        ...
        <div class="modal-body">
          <form @submit.prevent="submitModal">
            <!-- Nama Periode: dropdown dari GET /api/semesters -->
            <div class="form-group">
              ...
              >
                <option value="">
                  {{ isLoadingSemesters ? 'Memuat...' : '-- Pilih Semester --' }}
                </option>
                <option
                  v-for="s in semesterOptions"
                  :key="s.id"
                  :value="s.id"
                  :class="{ 'option-used': s.hasReviewPeriod }"
                >
                  {{ s.name }}{{ s.hasReviewPeriod ? ' ⚠ (sudah ada review)' : '' }}
                </option>
              </select>
              <div v-if="errors.semesterId" class="error-message show">
                Semester wajib dipilih.
              </div>
              <!-- Warn if selected semester already has a review period -->
              <div
                v-if="form.semesterId && semesterOptions.find(s => s.id === form.semesterId)?.hasReviewPeriod"
                class="warning-message"
              >
                ⚠ Semester ini sudah memiliki periode review. Menyimpan akan gagal (konflik).
              </div>
            </div>
...
/* Red option for semesters that already have a review period */
:deep(option.option-used) {
  color: #dc2626;
  background: #fff5f5;
}
