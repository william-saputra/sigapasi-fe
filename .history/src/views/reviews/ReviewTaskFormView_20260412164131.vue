...
const quantitativeQuestions = computed<QuantitativeQuestion[]>(() => {
  const result: QuantitativeQuestion[] = []
  let runningNumber = 1

  for (const group of QUESTION_GROUPS) {
    for (const questionText of group.questions) {
      result.push({
        id: `${group.aspect}-${runningNumber}`,
        text: questionText,
        aspect: group.aspect,
...
for (const item of quantitativeQuestions.value) {
  questionScores[item.id] = 3
}

const form = reactive<{ comment: string }>({
  comment: '',
})
...
const questionsByGroup = computed(() => {
  return QUESTION_GROUPS.map((group) => ({
    sectionTitle: group.sectionTitle,
    questions: quantitativeQuestions.value.filter((q) => q.aspect === group.aspect),
  }))
})

function getAspectScore(aspect: AspectKey): number {
  const aspectQuestions = quantitativeQuestions.value.filter((q) => q.aspect === aspect)
      ...
      <div class="form-section">
        ...
        <div v-for="aspect in aspectFields" :key="aspect.key" class="slider-group">
          ...
          <div class="slider-row">
            <h4 class="section-title">A. Penilaian Kompetensi (42 Pertanyaan)</h4>

            <div v-for="group in questionsByGroup" :key="group.sectionTitle" class="group-block">
